...
import { useCart } from '@fafa/storefront';
import { saveCustomer, loadCustomer, saveLastOrder } from '@fafa/storefront';
...
const LocationPicker = dynamic(() => import('../../../web/components/onboarding/location-picker'), {
  ssr: false,
});
...
function CheckoutContent({ slug }: { slug: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/order/?id=${data.order.id}&slug=${slug}`);
    } catch {
...
export default function CheckoutPage() {
  return (
    <Suspense fallback={<div className="p-8 text-center text-white/50">Loading checkout...</div>}>
      <CheckoutRoute />
    </Suspense>
  );
}
