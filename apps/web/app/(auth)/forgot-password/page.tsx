  ...
  if (sent) {
    return (
      <div className="text-center animate-fade-in">
        ...
        <Link
          href="/login"
          className="inline-block mt-6 text-sm text-brand-500 font-semibold hover:text-brand-600 transition-colors"
        >
          ← Back to sign in
        </Link>
      </div>
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-5">
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full py-3 px-4 rounded-xl bg-brand-500 text-white font-semibold hover:bg-brand-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-[0.98]"
        >
